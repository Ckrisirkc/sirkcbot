...
class RoleRequests(commands.Cog):
    ...
    async def _get_role_list_message(self, ctx, channel : discord.TextChannel = None):
        ...
        if n == 0:
            ...
            return "__***REQUESTABLE ROLES ON THIS SERVER***__  ({number} roles)\n{roles}\n\nModify your roles{in_chan} with:\n`{p}request add ROLE`  or  `{p}req ROLE`\n`{p}request rem ROLE`  or  `{p}rem ROLE`".format(number=n, roles=msg, p=ctx.prefix, in_chan=in_chan)
